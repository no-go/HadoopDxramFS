...
// Compile: gcc -O2 -m64 -o ../libJNIFileDirect.so -fpic -shared -I/usr/lib/jvm/java-8-oracle/include/
// -I/usr/lib/jvm/java-8-oracle/include/linux ./JNIFileDirect.c

/* This is the implementation of access to log files via JNI and O_DIRECT, so that
 * the page cache in Linux is bypassed and data is written directly. Keep in
 * mind that all addresses have to be aligned to a given BLOCKSIZE (here
...
long retain_preceding_bytes(int fileID, long write_buffer, long buffer_offset, long length, long aligned_start_pos,
    ...
    while (1) {
        ...
        if (read_bytes > 0) {
            ...
            // Error
            printf("JNI error (JNIFileDirect, retain_preceding_bytes): could not read from file (%s).",
                strerror(errno));
            printf(" [FileID: %d], [Block addr.: 0x%lx], [Buffer addr.: 0x%lx]\n", fileID, aligned_start_pos,
                block_start);
            return -1;
        }
    }
...
JNIEXPORT jint JNICALL Java_de_hhu_bsinfo_dxutils_jni_JNIFileDirect_write(JNIEnv *env, jclass clazz, jint fileID,
    ...
    while (aligned_length > 0) {
        ...
        if (written_bytes == -1) {
            // Error
            printf("JNI error (JNIFileDirect, write): could not write to file (%s).", strerror(errno));
            printf(" [FileID: %d], [Block addr.: 0x%lx], [Buffer addr.: 0x%lx], [Length: %ld]\n",
                fileID, aligned_start_pos, block_start, aligned_length);
            return -1;
        }
    ...
    if (set_file_length == 1) {
        ...
        if (ftruncate(fileID, start_pos + length) == -1) {
            printf("JNI error (JNIFileDirect, write): could not truncate file (%s).", strerror(errno));
...
JNIEXPORT jint JNICALL Java_de_hhu_bsinfo_dxutils_jni_JNIFileDirect_read(JNIEnv *env, jclass clazz, jint fileID,
    ...
    int file_length = get_length(fileID);
    int ret = pread(fileID, (void*) buffer, aligned_length, aligned_start_pos);
    if (ret == -1 || ret != aligned_length && ret != file_length) {
        // Error
        printf("JNI error (JNIFileDirect, read): could not read from file (%s).", strerror(errno));
        printf(" [FileID: %d], [Block addr.: 0x%lx], [Buffer addr.: 0x%lx], [Length: %ld] [File len.: %ld] [Ret: %d]\n",
            fileID, aligned_start_pos, buffer, aligned_length, file_length, ret);
        return -1;
    }
